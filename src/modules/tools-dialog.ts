...
import { getSelectedItems as getZoteroSelectedItems, extractPaperContexts } from "./pdf-extract";
...
function createDialog(win: Window): void {
  ...
  closeWinBtn.addEventListener("mouseenter", () => { closeWinBtn.style.background = "#e81123"; closeWinBtn.style.color = "#fff"; });
  closeWinBtn.addEventListener("mouseleave", () => { closeWinBtn.style.background = "transparent"; closeWinBtn.style.color = "var(--fill-primary,#333)"; });
  ...
  body.setAttribute("style", "display:flex; flex-direction:column; flex:1; overflow:hidden;");
...
function renderSearchBody(win: Window, body: HTMLElement): void {
  ...
  searchInput.setAttribute("placeholder", "Search by title, author, keywords, or content...");
  searchInput.addEventListener("keydown", (ev: any) => {
    if (ev.key === "Enter") { ev.preventDefault(); void doSearch(win); }
  });
  ...
  emptyCell.setAttribute("style", "text-align:center; padding:50px 0; color:rgba(0,0,0,0.3); font-size:12px;");
  emptyCell.textContent = "Enter a search query \u2014 search by title, author name, keywords, or any text from your papers.";
...
function renderTagBody(win: Window, body: HTMLElement): void {
  ...
  subBody.setAttribute("style", "display:flex; flex-direction:column; flex:1; overflow:hidden;");
...
function renderCategorizeSection(win: Window, container: HTMLElement): void {
  ...
  actionRow.setAttribute("style", "padding:10px 14px; display:flex; gap:8px; flex-shrink:0;");
  ...
  placeholder.setAttribute("style", "text-align:center; padding:50px 0; color:rgba(0,0,0,0.25); font-size:12px;");
  placeholder.textContent = "Click \"Categorize My Library\" to group your papers by similarity.";
...
async function doCategorize(win: Window): Promise<void> {
  ...
  if (!running) {
    if (status) { status.textContent = "Ollama is not running. Start Ollama first."; status.style.color = "#c33"; }
    return;
  }

  _busy = true;
  if (btn) { btn.textContent = "Analyzing..."; (btn as any).disabled = true; btn.style.opacity = "0.5"; }
  if (status) { status.textContent = "Loading library papers..."; status.style.color = "rgba(0,0,0,0.45)"; }

  results.innerHTML = "";
  const loadEl = doc.createElement("div") as HTMLElement;
  loadEl.setAttribute("style", "text-align:center; padding:40px 0; color:rgba(0,0,0,0.4); font-size:12px;");
  ...
  try {
    ...
    if (allPapers.length === 0) {
      ...
      noItems.setAttribute("style", "text-align:center; padding:40px 0; color:rgba(0,0,0,0.35); font-size:12px;");
    ...
    if (status) status.textContent = `Found ${allPapers.length} paper(s). AI is categorizing...`;
    ...
    if (categories.length === 0) {
      ...
      noCategories.setAttribute("style", "text-align:center; padding:40px 0; color:rgba(0,0,0,0.35); font-size:12px;");
    ...
    for (const cat of categories) {
      ...
      headerRow.setAttribute("style", "display:flex; align-items:center; justify-content:space-between; margin-bottom:8px;");

      const catLabel = doc.createElement("div") as HTMLElement;
      catLabel.setAttribute("style", "display:flex; align-items:center; gap:8px;");
      ...
      countLabel.setAttribute("style", "font-size:11px; color:rgba(0,0,0,0.4);");
      ...
      for (const p of cat.papers) {
        ...
        info.setAttribute("style", "overflow:hidden; text-overflow:ellipsis; white-space:nowrap;");
    ...
    if (status) { status.textContent = `${categories.length} categories found across ${allPapers.length} paper(s).`; status.style.color = "rgba(0,0,0,0.45)"; }
  } catch (e: any) {
    results.innerHTML = "";
    const errEl = doc.createElement("div") as HTMLElement;
    errEl.setAttribute("style", "text-align:center; padding:20px 0; color:#c33; font-size:12px;");
    errEl.textContent = `Error: ${String(e)}`;
    results.appendChild(errEl);
    if (status) { status.textContent = "Categorization failed."; status.style.color = "#c33"; }
  } finally {
    _busy = false;
    if (btn) { btn.textContent = "Categorize My Library"; (btn as any).disabled = false; btn.style.opacity = "1"; }
  }
}
...
function renderSearchTagSection(win: Window, container: HTMLElement): void {
  ...
  searchInput.addEventListener("keydown", (ev: any) => {
    if (ev.key === "Enter") { ev.preventDefault(); void doTagSearch(win); }
  });
  ...
  const placeholder = doc.createElement("div") as HTMLElement;
  placeholder.setAttribute("style", "text-align:center; padding:50px 0; color:rgba(0,0,0,0.25); font-size:12px;");
  placeholder.textContent = "Search for papers to see them here, then suggest tags.";
...
async function doTagSearch(win: Window): Promise<void> {
  ...
  if (status) { status.textContent = "Searching..."; status.style.color = "rgba(0,0,0,0.45)"; }

  resultsWrap.innerHTML = "";
  const loadEl = doc.createElement("div") as HTMLElement;
  loadEl.setAttribute("style", "text-align:center; padding:30px 0; color:rgba(0,0,0,0.4); font-size:12px;");
  ...
  try {
    ...
    if (results.length === 0) {
      ...
      emptyEl.setAttribute("style", "text-align:center; padding:40px 0; color:rgba(0,0,0,0.3); font-size:12px;");
    ...
    header.setAttribute("style", "padding:10px 14px 6px; font-size:11px; font-weight:600; color:var(--fill-primary,#555);");
    ...
    const table = doc.createElement("table") as HTMLElement;
    table.setAttribute("style", css.table + "; margin:0 10px; width:calc(100% - 20px);");
    ...
    for (const [i, r] of results.entries()) {
      ...
      tr.addEventListener("mouseenter", () => { tr.style.background = "rgba(74,144,217,0.06)"; });
      tr.addEventListener("mouseleave", () => { tr.style.background = "transparent"; });

      const cells = [
        [String(i + 1), ""],
        [r.title || "Untitled", "font-weight:500; max-width:280px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;"],
        [r.authors || "\u2014", "max-width:160px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;"],
        [r.year || "\u2014", ""],
      ];
    ...
    suggestRow.setAttribute("style", "padding:10px 14px; display:flex; gap:8px; flex-shrink:0;");
    ...
    if (status) { status.textContent = `${results.length} paper(s) found. Click "Suggest Tags" to generate tags.`; status.style.color = "rgba(0,0,0,0.45)"; }
  } catch (e: any) {
    resultsWrap.innerHTML = "";
    const errEl = doc.createElement("div") as HTMLElement;
    errEl.setAttribute("style", "text-align:center; padding:20px 0; color:#c33; font-size:12px;");
    ...
    if (status) { status.textContent = "Search failed."; status.style.color = "#c33"; }
  } finally {
    _busy = false;
  }
}
...
async function doSuggestTags(win: Window): Promise<void> {
  ...
  _busy = true;
  _selectedTags = new Set();
  if (suggestBtn) { suggestBtn.textContent = "Generating..."; (suggestBtn as any).disabled = true; suggestBtn.style.opacity = "0.5"; }
  if (status) { status.textContent = "Analyzing paper(s) and generating tags..."; status.style.color = "rgba(0,0,0,0.45)"; }

  tagsSection.style.display = "block";
  tagsSection.innerHTML = "";
  const loadingEl = doc.createElement("div") as HTMLElement;
  loadingEl.setAttribute("style", "text-align:center; padding:20px 0; color:rgba(0,0,0,0.4); font-size:12px;");
  ...
  try {
    ...
    if (uniqueTags.length === 0) {
      ...
      noTags.setAttribute("style", "text-align:center; padding:20px 0; color:rgba(0,0,0,0.35); font-size:12px;");
      ...
      const header = doc.createElement("div") as HTMLElement;
      header.setAttribute("style", "font-size:11px; font-weight:600; color:var(--fill-primary,#555); margin-bottom:8px;");
      ...
      chipContainer.setAttribute("style", "display:flex; flex-wrap:wrap; gap:6px;");
      ...
      if (status) { status.textContent = `${uniqueTags.length} tag(s) suggested. Select and apply.`; status.style.color = "rgba(0,0,0,0.45)"; }
    }
  } catch (e: any) {
    tagsSection.innerHTML = "";
    const errEl = doc.createElement("div") as HTMLElement;
    errEl.setAttribute("style", "text-align:center; padding:14px 0; color:#c33; font-size:12px;");
    errEl.textContent = `Error: ${String(e)}`;
    tagsSection.appendChild(errEl);
    if (status) { status.textContent = "Tag generation failed."; status.style.color = "#c33"; }
  } finally {
    _busy = false;
    if (suggestBtn) { suggestBtn.textContent = "Suggest Tags for These Papers"; (suggestBtn as any).disabled = false; suggestBtn.style.opacity = "1"; }
  }
}
...
async function doApplyTags(win: Window): Promise<void> {
  ...
  if (applyBtn) { applyBtn.textContent = "Applying..."; (applyBtn as any).disabled = true; applyBtn.style.opacity = "0.5"; }
  ...
  try {
    ...
    if (applyBtn) { applyBtn.textContent = "Apply Selected Tags"; (applyBtn as any).disabled = false; applyBtn.style.opacity = "1"; }
    if (status) { status.textContent = `Error applying tags: ${String(e)}`; status.style.color = "#c33"; }
  }
}
...
let _allCompareItems: { item: any; title: string; authors: string; year: string }[] = [];
...
function renderCompareBody(win: Window, body: HTMLElement): void {
  ...
  // Action row
  const actionRow = doc.createElement("div") as HTMLElement;
  actionRow.setAttribute("style", "padding:6px 14px; display:flex; gap:8px; align-items:center; flex-shrink:0; border-bottom:1px solid rgba(0,0,0,0.08);");

  const selCount = doc.createElement("span") as HTMLElement;
  selCount.id = "zotero-ai-compare-count";
  selCount.setAttribute("style", "font-size:11px; color:rgba(0,0,0,0.45); flex:1;");
  ...
  listWrap.setAttribute("style", "flex:1; overflow-y:auto; border-bottom:1px solid rgba(0,0,0,0.08);");

  const loadingEl = doc.createElement("div") as HTMLElement;
  loadingEl.setAttribute("style", "text-align:center; padding:40px 0; color:rgba(0,0,0,0.4); font-size:12px;");
  ...
  resultsArea.setAttribute("style", "flex:1; overflow-y:auto; padding:0; display:none;");
...
async function loadCompareList(win: Window): Promise<void> {
  ...
  try {
    ...
    if (_allCompareItems.length === 0) {
      ...
      empty.setAttribute("style", "text-align:center; padding:40px 0; color:rgba(0,0,0,0.3); font-size:12px;");
    ...
    if (status) status.textContent = `${_allCompareItems.length} paper(s) in library`;
  } catch (e: any) {
    listWrap.innerHTML = "";
    const err = doc.createElement("div") as HTMLElement;
    err.setAttribute("style", "text-align:center; padding:20px 0; color:#c33; font-size:12px;");
...
function buildCompareRows(doc: Document, listWrap: HTMLElement): void {
  ...
  for (const [i, p] of _allCompareItems.entries()) {
    ...
    numEl.setAttribute("style", "min-width:22px; color:rgba(0,0,0,0.35); font-size:11px; flex-shrink:0; text-align:right;");

    const titleEl = doc.createElement("span") as HTMLElement;
    titleEl.textContent = p.title || "Untitled";
    titleEl.setAttribute("style", "flex:1; font-weight:500; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; font-size:12px;");

    const authorsEl = doc.createElement("span") as HTMLElement;
    authorsEl.textContent = p.authors || "\u2014";
    authorsEl.setAttribute("style", "max-width:140px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; font-size:11px; color:rgba(0,0,0,0.5); flex-shrink:0;");

    const yearEl = doc.createElement("span") as HTMLElement;
    yearEl.textContent = p.year || "";
    yearEl.setAttribute("style", "min-width:35px; font-size:11px; color:rgba(0,0,0,0.4); flex-shrink:0; text-align:right;");
...
function filterCompareList(query: string): void {
  ...
  for (const entry of _compareRowEls) {
    if (!lower) { entry.row.style.display = "flex"; continue; }
...
async function doCompare(win: Window): Promise<void> {
  ...
  _busy = true;
  if (btn) { btn.textContent = "Comparing..."; btn.setAttribute("disabled", "true"); btn.style.opacity = "0.5"; }
  if (status) { status.textContent = "Extracting paper content..."; status.style.color = "rgba(0,0,0,0.45)"; }
  ...
  try {
    ...
    if (paperContexts.length < 2) {
      results.innerHTML = "";
      const errEl = doc.createElement("div") as HTMLElement;
      errEl.setAttribute("style", "text-align:center; padding:30px 0; color:#c33; font-size:12px;");
    ...
    if (status) status.textContent = `AI is comparing ${paperContexts.length} papers...`;

    results.innerHTML = "";

    const headerEl = doc.createElement("div") as HTMLElement;
    headerEl.setAttribute("style", "padding:10px 14px 8px; font-size:11px; font-weight:600; color:var(--fill-primary,#555);");
    ...
    for (const [i, p] of paperContexts.entries()) {
      const row = doc.createElement("div") as HTMLElement;
      row.setAttribute("style", "padding:2px 14px; font-size:11px; color:var(--fill-primary,#444);");
      row.textContent = `[${i + 1}] ${p.title}${p.authors ? ` \u2014 ${p.authors}` : ""}${p.year ? ` (${p.year})` : ""}`;
      results.appendChild(row);
    }

    const divider = doc.createElement("hr") as HTMLElement;
    divider.setAttribute("style", "border:none; border-top:1px solid rgba(0,0,0,0.08); margin:10px 14px;");
    ...
    await LibTools.comparePapers(
      ...
      (token) => {
        if (firstToken) { outputEl.textContent = ""; firstToken = false; }
        outputEl.textContent += token;
      },
    );

    appendBackBtn(doc, results, listWrap);
    if (status) { status.textContent = `Compared ${paperContexts.length} papers.`; status.style.color = "rgba(0,0,0,0.45)"; }
  } catch (e: any) {
    results.innerHTML = "";
    const errEl = doc.createElement("div") as HTMLElement;
    errEl.setAttribute("style", "text-align:center; padding:20px 0; color:#c33; font-size:12px;");
    ...
    results.appendChild(errEl);
    appendBackBtn(doc, results, listWrap);
    if (status) { status.textContent = "Comparison failed."; status.style.color = "#c33"; }
  } finally {
    _busy = false;
    if (btn) { btn.textContent = "Compare Selected"; btn.removeAttribute("disabled"); btn.style.opacity = "1"; }
  }
}

function appendBackBtn(doc: Document, container: HTMLElement, listWrap: HTMLElement | null): void {
...
async function doSearch(win: Window): Promise<void> {
  ...
  loadCell.setAttribute("style", "text-align:center; padding:40px 0; color:rgba(0,0,0,0.4); font-size:12px;");
  ...
  if (answerArea) { answerArea.style.display = "none"; answerArea.textContent = ""; answerArea.style.borderLeftColor = "#4a90d9"; }
  ...
  try {
    ...
    if (results.length === 0) {
      ...
      const emptyCell = doc.createElement("td") as HTMLElement;
      emptyCell.setAttribute("colspan", "5");
      emptyCell.setAttribute("style", "text-align:center; padding:40px 0; color:rgba(0,0,0,0.3); font-size:12px;");
      ...
      for (const [i, r] of results.entries()) {
        ...
        for (const [ci, [text, extra]] of cells.entries()) {
          ...
          if (ci === 4) {
            ...
            bar.setAttribute("style", "display:flex; align-items:center; gap:6px;");

            const track = doc.createElement("div") as HTMLElement;
            track.setAttribute("style", "width:60px; height:8px; background:rgba(0,0,0,0.08); border-radius:4px; overflow:hidden;");
            const fill = doc.createElement("div") as HTMLElement;
            const color = pct > 70 ? "#43a047" : pct > 40 ? "#fb8c00" : "#e53935";
            fill.setAttribute("style", `height:100%; width:${pct}%; background:${color}; border-radius:4px;`);
            track.appendChild(fill);

            const label = doc.createElement("span") as HTMLElement;
            label.textContent = `${pct}%`;
            label.setAttribute("style", "font-size:10px; color:rgba(0,0,0,0.4); min-width:28px;");
    ...
    if (status) { status.textContent = `${results.length} result(s) found`; status.style.color = "rgba(0,0,0,0.45)"; }
  } catch (e: any) {
    tbody.innerHTML = "";
    ...
    errCell.setAttribute("style", "text-align:center; padding:30px 0; color:#c33; font-size:12px;");
    errCell.textContent = `Error: ${String(e)}`;
    errRow.appendChild(errCell);
    tbody.appendChild(errRow);

    if (answerArea) { answerArea.style.display = "block"; answerArea.textContent = `Error: ${String(e)}`; answerArea.style.borderLeftColor = "#c33"; }
    if (status) { status.textContent = "Search failed"; status.style.color = "#c33"; }
  } finally {
    _busy = false;
    input.value = "";
...
function makeDraggable(win: Window, panel: HTMLElement, handle: HTMLElement): void {
  let dragging = false;
...
function injectStyles(doc: Document): void {
  ...
  el.textContent = "@keyframes zotero-ai-spin { to { transform: rotate(360deg); } }";
...
const css = {
  ...
  dialog: [
    "width:740px", "max-width:92vw", "max-height:84vh",
  ...
  tagChip: (selected: boolean) => [
    ...
    "border-radius:16px",
    "background:" + (selected ? "#4a90d9" : "var(--material-background,#fff)"),
