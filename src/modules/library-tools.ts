...
export function formatChatAsHTML(
  ...
  lines.push(
    `<p><em>Generated by Zotero Infinity on ${new Date().toLocaleDateString()}</em></p>`,
  );
  ...
  if (papers.length > 0) {
    ...
    for (const p of papers) {
      lines.push(
        `<li><strong>${escHTML(p.title)}</strong>${p.authors ? ` — ${escHTML(p.authors)}` : ""}${p.year ? ` (${p.year})` : ""}</li>`,
      );
    }
    lines.push("</ul><hr/>");
  }
  ...
  for (const m of messages) {
    ...
    if (m.role === "user") {
      lines.push(
        `<p><strong>You:</strong></p><blockquote>${escHTML(m.content)}</blockquote>`,
      );
    ...
    try {
      ...
      try {
        ...
        for (const att of pdfs) {
          ...
          if (t) {
            fullText = t.toLowerCase().slice(0, 20000);
            break;
          }
        }
      } catch {
...
export async function librarySearchWithAI(
  ...
  if (results.length === 0) {
    return {
      answer: "No papers found in your library matching that query.",
      results: [],
    };
  }
  ...
  const messages: ChatMessage[] = [
    {
      role: "system",
      content:
        "You are a research paper classifier. Given a paper's metadata and text, suggest 3-7 descriptive tags. Output ONLY the tags as a comma-separated list, nothing else. Tags should be lowercase, concise (1-3 words each), and cover the paper's topic, methodology, and field.",
    },
...
export async function applyTags(item: any, tags: string[]): Promise<void> {
  ...
  await item.saveTx();
  Zotero.debug(
    `[zotero-local-ai] Applied ${tags.length} tags to "${item.getField?.("title")}"`,
  );
}
